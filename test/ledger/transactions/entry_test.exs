defmodule TidelandLedger.Transactions.EntryTest do
  ...
  describe "changeset/2" do
    ...
    test "requires date" do
      ...
      assert :required in extract_entry_errors(changeset).date
    end
    ...
    test "requires description" do
      ...
      refute changeset.valid?
      assert :required in extract_entry_errors(changeset).description
    end
    ...
    test "requires created_by_id" do
      ...
      refute changeset.valid?
      assert :required in extract_entry_errors(changeset).created_by_id
    end
    ...
    test "validates description length" do
      ...
      refute changeset.valid?
      assert {:min_length, 3} in extract_entry_errors(changeset).description

      attrs = valid_entry_attrs(%{description: String.duplicate("a", 501)})
      changeset = Entry.changeset(%Entry{}, attrs)

      refute changeset.valid?
      assert {:max_length, 500} in extract_entry_errors(changeset).description
    end
    ...
    test "validates reference length" do
      ...
      refute changeset.valid?
      assert {:max_length, 50} in extract_entry_errors(changeset).reference
    end
    ...
    test "rejects future dates" do
      ...
      refute changeset.valid?
      assert :future_date_not_allowed in extract_entry_errors(changeset).date
    end
    ...
    test "rejects dates exceeding backdate limit" do
      ...
      refute changeset.valid?
      assert {:exceeds_backdate_limit, 365} in extract_entry_errors(changeset).date
    end
    ...
    test "requires positions" do
      ...
      refute changeset.valid?
      assert :required in extract_entry_errors(changeset).positions
    end
    ...
    test "requires at least 2 positions" do
      ...
      refute changeset.valid?
      assert :insufficient_positions in extract_entry_errors(changeset).positions
    end
    ...
    test "validates maximum positions" do
      ...
      refute changeset.valid?
      assert {:exceeds_max_positions, 100} in extract_entry_errors(changeset).positions
    end
    ...
    test "validates positions must balance" do
      ...
      refute changeset.valid?
      assert :transaction_not_balanced in extract_entry_errors(changeset).positions
    end
    ...
    test "validates unique accounts in positions" do
      ...
      refute changeset.valid?
      assert :duplicate_accounts in extract_entry_errors(changeset).positions
    end
  ...
  describe "post_changeset/2" do
    ...
    test "cannot post already posted entry", %{entry: entry} do
      ...
      refute changeset.valid?
      assert :already_posted in extract_entry_errors(changeset).status
    end
  ...
  describe "void_changeset/3" do
    ...
    test "requires void reason", %{entry: entry} do
      ...
      refute changeset.valid?
      assert :required in extract_entry_errors(changeset).void_reason
    end
    ...
    test "validates void reason length", %{entry: entry} do
      ...
      refute changeset.valid?
      assert {:min_length, 5} in extract_entry_errors(changeset).void_reason

      long_reason = String.duplicate("a", 501)
      changeset = Entry.void_changeset(entry, 123, long_reason)

      refute changeset.valid?
      assert {:max_length, 500} in extract_entry_errors(changeset).void_reason
    end
    ...
    test "cannot void draft entry", %{entry: entry} do
      ...
      refute changeset.valid?
      assert :not_posted in extract_entry_errors(changeset).status
    end
  ...
  defp extract_entry_errors(changeset) do
    changeset.errors
